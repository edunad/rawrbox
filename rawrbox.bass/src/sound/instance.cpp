...
namespace rawrbox {
	...
	std::vector<float> SoundInstance::getFFT(int bass_length) const {
		...
		switch (bass_length) {
			...
			case 4096:
				...
				break;
			default:
				RAWRBOX_CRITICAL("Unknown FFT length {}, should be power of 2! Check: http://bass.radio42.com/help/html/a13cfef0-1056-bb94-81c4-a4fdf21bd463.htm", bass_length);
		}
	...
	void SoundInstance::setBeatSettings(float bandwidth, float center_freq, float release_time) {
		if ((this->_flags & SoundFlags::BEAT_DETECTION) == 0) RAWRBOX_CRITICAL("Load flag BEAT_DETECTION not set!");
		if (!this->isCreated()) return;
