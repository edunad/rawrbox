	...
	float4 boneTransform(uint4 indices, half4 weight, float4 position) {
	...
	TransformedData applyPosTransforms(float4x4 proj, float4 a_position, half2 a_texcoord0) {
		...
		if (DisplacementPower != 0. && DisplacementTexture != 0.) {
			data.pos.y += g_Textures[DisplacementTexture].SampleLevel(g_Sampler, half3(a_texcoord0, 0), 0).x * DisplacementPower;
		}
		#endif
		// ----
	...
	TransformedData applyPosTransforms(float4 a_position, half2 a_texcoord0) {
		return applyPosTransforms(Camera.worldViewProj, a_position, a_texcoord0);
	}

	TransformedData applyPosTransforms(float3 a_position, half2 a_texcoord0) {
		return applyPosTransforms(Camera.worldViewProj, float4(a_position, 1.0), a_texcoord0);
	}

	TransformedData applyPosTransforms(float4x4 proj, float3 a_position, half2 a_texcoord0) {
