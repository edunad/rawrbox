...
namespace rawrbox {
	...
	public:
		AnimationSampler(size_t index, ozz::animation::Animation* anim) : _animation(anim), _index(index) {
		...
		virtual bool tick(float deltaTime) {
			...
			float newTime = std::fmodf(this->getTime() + (timeToAdd / this->getDuration()), 1.F);
