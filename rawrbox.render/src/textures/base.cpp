...
namespace rawrbox {
	...
	const std::vector<uint8_t>& TextureBase::getPixels(size_t index) const {
		if (index > this->_data.frames.size()) RAWRBOX_CRITICAL("Pixel data from frame index {} not found", index);
	...
	void TextureBase::upload(Diligent::TEXTURE_FORMAT format, bool dynamic) {
		...
		if (!this->_data.valid()) RAWRBOX_CRITICAL("Cannot upload invalid image data");
		if (this->_data.total() > 2048U) RAWRBOX_CRITICAL("Cannot upload more than 2048 image frames");
		...
		if (this->_tex == nullptr) RAWRBOX_CRITICAL("Failed to create texture '{}'", this->_name);
