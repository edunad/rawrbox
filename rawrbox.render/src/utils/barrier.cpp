...
namespace rawrbox {
	...
	void BarrierUtils::barrier(const std::vector<Diligent::StateTransitionDesc>& resources) {
		...
		for (const auto& barrier : resources) {
			auto fnd = _barrierCache.find(barrier.pResource);
			if (fnd != _barrierCache.end() && fnd->second == barrier.NewState) continue; //  Was already executed

			states.emplace_back(barrier);
			_barrierCache[barrier.pResource] = barrier.NewState;
		}
