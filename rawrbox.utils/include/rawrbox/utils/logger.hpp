...
namespace rawrbox {
	...
	public:
		...
		void warn(fmt::format_string<T...> fmt, T&&... args) {
			...
			fmt::print("[{} ▓ {}]: {}\n", fmt::format(fmt::fg(fmt::color::yellow), "WARN"), fmt::format(fmt::fg(fmt::color::yellow), this->_title), fmt::format(fmt::fg(fmt::color::yellow), str));
		}

		template <typename... T>
		cpptrace::runtime_error error(fmt::format_string<T...> fmt, T&&... args) {
			auto str = fmt::format(fmt, std::forward<T>(args)...);
			return cpptrace::runtime_error(fmt::format("{}: {}", this->_title, str));
		}
