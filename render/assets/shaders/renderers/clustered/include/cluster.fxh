...
#define LIGHT_GRID
Buffer<uint2 /*format=r32i*/> g_clusterLightGrid; // Read-only
#endif
...
Buffer<uint /*format=r32i*/> g_globalIndex; // Read-only
#define ATOMIC
#endif
...
#ifdef READ_LIGHT_GRID
LightGrid getLightGrid(uint cluster) {
    uint2 gridvec = g_clusterLightGrid[cluster];
...
uint getClusterIndex(float4 position) {
    ...
    uint3 indices = uint3(uint2(position.xy / g_ClusterSize), zIndex);
