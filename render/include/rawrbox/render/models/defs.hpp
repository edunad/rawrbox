...
namespace rawrbox {
	struct VertexData {
		...
		explicit VertexData(const rawrbox::Vector3f& _pos,
		    const rawrbox::Vector4f& _uv = {}, const rawrbox::Color cl = rawrbox::Colors::White()) : position(_pos), uv(_uv), color(cl) /*, _ori_pos(position)*/ {}

		explicit VertexData(const rawrbox::Vector3f& _pos,
		    const rawrbox::Vector2f& _uv = {}, const rawrbox::Color cl = rawrbox::Colors::White()) : position(_pos), uv(_uv), color(cl) /*, _ori_pos(position)*/ {}

		explicit VertexData(const rawrbox::Vector3f& _pos,
		    const rawrbox::Vector2f& _uv = {},
		    const std::array<uint32_t, 2>& _normal = {}, const rawrbox::Color cl = rawrbox::Colors::White()) : position(_pos), uv(_uv), color(cl) /*, normal(_normal), _ori_pos(position), _ori_norm(normal[0])*/ {}
		...
		static std::vector<Diligent::LayoutElement> vLayout(bool normals = false, bool bones = false) {
			...
			if (normals) {
				// layout.emplace_back(3, 0, 4, Diligent::VT_FLOAT32, false);
