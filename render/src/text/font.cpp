...
namespace rawrbox {
	...
	std::unique_ptr<rawrbox::Glyph> Font::bakeGlyphAlpha(uint16_t codePoint) {
		...
		auto glyph = std::make_unique<rawrbox::Glyph>();
	...
	rawrbox::TextureAtlas* Font::getAtlasTexture(rawrbox::Glyph* g) const {
		...
		return rawrbox::TextEngine::getAtlas(g->atlasID).get();
	}

	void Font::render(const std::string& text, const rawrbox::Vector2f& pos, std::function<void(rawrbox::Glyph*, float, float, float, float)> renderGlyph) {
