...
namespace rawrbox {
	ParticleSystem::ParticleSystem(rawrbox::TextureAtlas& spriteAtlas) : _atlas(&spriteAtlas){};

	int32_t ParticleSystem::particleSortFn(const void* _lhs, const void* _rhs) {
	...
	// UTILS -----
	rawrbox::TextureAtlas* ParticleSystem::getTexture() const { return this->_atlas; }

	void ParticleSystem::upload() { this->_material->upload(); }

	rawrbox::Emitter& ParticleSystem::addEmitter(rawrbox::Emitter em) {
		return *this->_emitters.emplace_back(std::make_unique<rawrbox::Emitter>(em));
	}

	void ParticleSystem::removeEmitter(rawrbox::Emitter* em) {
	...
	rawrbox::Emitter& ParticleSystem::get(size_t indx) const {
	...
	void ParticleSystem::draw() {
		...
		for (auto& em : this->_emitters) {
			pos += em->draw(pos, max, particleSort.data(), vertices);
		}
